...
<body>
    ...
    <script>
        document.getElementById('loginForm').addEventListener('submit', function(e) {
            ...
            if (email === 'demo@kongu.edu' && password === 'demo123') {
                // Create demo user data
                const demoUser = {
                    firstName: 'Demo',
                    lastName: 'User',
                    rollNumber: '23CSE999',
                    email: 'demo@kongu.edu',
                    mobile: '9876543210',
                    department: 'CSE',
                    year: '3',
                    interests: ['music', 'coding', 'robotics']
                };
                
                localStorage.setItem('konguUser', JSON.stringify(demoUser));
            ...
            const savedUser = localStorage.getItem('konguUser');
            
            if (savedUser) {
                const userData = JSON.parse(savedUser);
                
                // Check credentials (simplified for demo)
                if ((email === userData.email || email === userData.rollNumber) && password.length >= 8) {
                    localStorage.setItem('konguLoggedIn', 'true');
                    
                    // Remember me functionality
                    if (document.getElementById('remember').checked) {
                        localStorage.setItem('konguRemember', 'true');
                    }
                    
                    alert('Login successful! Welcome back, ' + userData.firstName + '!');
                    window.location.href = 'index.html';
                } else {
                    alert('Invalid email/roll number or password. Please try again.');
                }
            } else {
        ...
        if (localStorage.getItem('konguRemember') === 'true') {
            const savedUser = localStorage.getItem('konguUser');
            if (savedUser) {
                const userData = JSON.parse(savedUser);
        ...
        if (localStorage.getItem('konguLoggedIn') === 'true') {
